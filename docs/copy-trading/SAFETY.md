...
## 69. P0 feed-replay containment and safe restart (2026-08-05)

**Production symptom:** after a controlled paper-bot restart, the process remained alive with zero
logged exceptions but generated thousands of old-trade skip events in minutes, issued 16 paper
sells, and delayed the nominal five-minute TTP/equity sweep to 8m02s. Linux intermittently showed
the process waiting on SQLite journal commits because every falsely-new row took the normal
fail-safe persistence path. An alive PID was therefore not sufficient evidence of a healthy loop.

**Containment:** `data/watchdog_paused` was created before sending SIGTERM to the single confirmed
bot PID. This uses the existing reversible operator mechanism and prevents the two-minute watchdog
from racing the repair. The Telegram approval listener and Go OMS are separate services and were
not stopped. No database row, position, wallet status, risk value, or strategy parameter was
manually edited during containment.

**Code safety boundary:** live polling now passes its in-memory known trade IDs through
`bot.fetch_direct_feed()` to `fetch_all_wallets_concurrent()` and `fetch_wallet_trades()`. Offset
pagination stops only after a complete page overlaps known history. Without that argument, the
API client's behavior is unchanged, keeping validation/backfill tools isolated from the live-loop
optimization. The stable raw-record trade ID is generated by one shared helper so pagination and
normalization cannot disagree about where the boundary is.

**Restart gate:** keep the watchdog pause in place until the tested commit is present on EC2 and a
single paper-mode process is started. Verify the DB-backed roster and EC2-only OMS override remain
intact, then require bounded replay counters, zero post-start errors, `kill_switch_active=0`, and
two successful TTP marks before declaring P0 complete.
