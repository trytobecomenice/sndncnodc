...
3. event-time termination classifier is deployed and `UNKNOWN <= 1%`;
4. frozen latency ECDF, entity clustering version, termination classifier,
   dataset schema, protocol, evaluator, MC seed/code, and reference parameter
   table are present in one SHA-256 manifest;
5. selected and randomized matched-control cohorts are fixed.
